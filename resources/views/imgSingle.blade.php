...
<div class="register-box">
  <div class="card card-outline card-primary">
    ...
    <div class="card-body">
     ...
     <img src={{ URL::to('/'). '/images/'.$data->img_path }} alt="wrong">
      <form action="{{ route('updateImage') }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="input-group mb-3">
          <input type="hidden" name="id" value="{{ $data->id }}">
          <input type="text" class="form-control @error('name') is-invalid @enderror " name="name" placeholder="Full name" value="{{$data->name }}">
